...
- [x] **T1.1.1** 创建 `server/WaveGenerator.js`
...
- [x] **T2.1.1** 在 `server/index.js` 顶部导入 `WaveGenerator`
  - `import { generateWaves } from './WaveGenerator.js'`

- [x] **T2.1.2** 在 `launchBattle()` 中调用 `generateWaves(room.dungeonId)`
...
- [x] **T2.1.3** 更新 `battle:init` 广播 payload，新增 `waves` 字段
  - `io.to(room.id).emit('battle:init', { dungeonId, seed, snapshots, roomId, waves })`

- [x] **T2.1.4** 更新 `stmts.saveBattleState.run(...)` 调用，传入含 `waves` 的完整 battleState
...
- [x] **T3.1.1** 更新 `battle:restore`（从数据库读取的分支，`server/rooms.js` 中的 `getBattleState` 路径）
...
- [x] **T3.1.2** 更新内存重连分支（`engine && engine.battleState` 分支，`server/index.js` 中的 `battle:init` resend）
...
- [x] **T4.1.1** 检查 `battle:wave_progress` 处理器
...
- [x] **T5.1.1** 修改 `src/systems/MultiplayerDungeonAdapter.js` 的 `start(initData)` 方法
...
- [x] **T5.1.2** 修改 `startDungeonMultiplayer` 调用处，传入 `waves` option
...
- [x] **T6.1.1** 修改 `src/systems/DungeonCombatSystem.js` 的 `startDungeonMultiplayer(dungeonId, party, options)` 方法
...
- [x] **T6.1.2** 修改 `startNextEncounter()` 方法，优先使用注入的波次数据
...
- [x] **T7.1.1** 在 `tests/unit/server/` 下新建 `WaveGenerator.test.js`
...
- [x] **T7.2.1** 在现有服务端测试或新建测试文件中为 `launchBattle` 补充测试
...
- [x] **T7.3.1** 为 `MultiplayerDungeonAdapter.start()` 补充测试（在 `tests/unit/` 下）
...
- [x] **T7.4.1** 在 `tests/unit/` 下补充 `DungeonCombatSystem` 波次注入测试
